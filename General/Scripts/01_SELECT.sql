 ...
 * 
 ** DATE -> TIMESTAMP 바꾸면 ms 단위 + 지역 정보를 추가로 얻어옴
**/
...
/* 연결 연산자(||) 
 * - 두 컬럼값 또 리터럴을 하나의 문자열로 연결할 때 사용
 * */
